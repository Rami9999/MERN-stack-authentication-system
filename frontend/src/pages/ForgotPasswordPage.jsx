            ...
            {!isSubmitted ? (           
                <form onSubmit={handleSubmit}>
                    ...
                    <motion.button
                        ...
                        disabled={isLoading}
                    >
                        {isLoading ? <Loader className='w-6 h-6 animate-spin mx-auto' size={24} /> : "Send"}
                    </motion.button>
                    ...
                    <p className='text-gray-300 mb-6'>
                        If an account exists for {formData.email}, you will receive a password reset link shorlty.
                    </p>
                </div>)}
        </div>
        <div className='px-8 py-4 bg-gray-900 bg-opacity-50 flex justify-center'>
            <Link to={'/login'} className=' text-sm text-green-400 hover:underline flex items-center'><ArrowLeft className='w-4 h-4 mr-2'/>Back To Login</Link>
        </div>
    </motion.div>
  )
}
